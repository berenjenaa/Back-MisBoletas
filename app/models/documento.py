...
class Documento(Base):
    ...
    NombreArchivo = Column(
        "nombrearchivo", String(255), nullable=False
    )  # Nombre original del archivo
    URL_GCS = Column(
        "url_gcs", String(500), nullable=False
    )  # URL completa de GCS (público o firmado)
    BlobName = Column(
        "blob_name", String(500), nullable=False, unique=True
    )  # Nombre del blob en GCS (para eliminación)
